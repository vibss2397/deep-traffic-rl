...
const ENEMY_SPAWN_RATE = 0.015; // Reduced spawn rate for better playability
...
function handleInput() {
  // Make lane switching nearly instantaneous
  // Just snap to the target lane position
  player.x = getLaneX(player.targetLane, 1.0);
  player.lane = player.targetLane;
  
  // Optional: Add a very small visual effect for lane changes
...
function getLaneX(lane, yPercent) {
  ...
  // Calculate the center of each lane properly
  const laneWidthAtY = roadWidthAtY / LANE_COUNT;
  const leftEdge = horizonX - (roadWidthAtY / 2);
  
  // Lane centers (not lane dividers)
  return leftEdge + (lane * laneWidthAtY) + (laneWidthAtY / 2);
}
...
function spawnEnemies() {
  ...
  if (random() < ENEMY_SPAWN_RATE) {
    ...
    // Check adjacent lanes too - don't create impassable walls
    let hasPassableGap = false;
    if (laneFree) {
      // Check all lanes to see if there's always at least one free lane
      let occupiedLanes = new Array(LANE_COUNT).fill(false);
      
      // Mark which lanes have cars near the spawn area
      for (let enemy of enemies) {
        if (enemy.yPercent < 0.2) {
          occupiedLanes[enemy.lane] = true;
        }
      }
      
      // Mark the lane we're about to fill
      occupiedLanes[lane] = true;
      
      // Make sure there's at least one free lane
      hasPassableGap = occupiedLanes.includes(false);
      
      // If we'd create an impassable wall, don't spawn this enemy
      if (!hasPassableGap) {
        laneFree = false;
      }
    }
...
function checkCollisions() {
  // Improved collision detection that allows for "squeezing through" between cars
  for (let enemy of enemies) {
    // Only check enemies that are near the player's y position
    if (enemy.yPercent > 0.9) {
      // Calculate horizontal distance between cars
      const distX = Math.abs(enemy.x - player.x);
      
      // Calculate combined half-widths with some margin for "squeezing through"
      const combinedHalfWidth = (PLAYER_WIDTH * 0.4) + (enemy.width * enemy.scale * 0.4);
      
      // Vertical collision detection with actual car bodies
      const playerTopY = player.y - PLAYER_HEIGHT/2;
      const enemyBottomY = enemy.y + (enemy.height * enemy.scale / 2);
      
      // Only collide if cars are truly overlapping (with reduced hitbox)
      if (distX < combinedHalfWidth && enemyBottomY > playerTopY) {
        gameOver = true;
        break;
      }
    }
...
function keyPressed() {
  // Always process lane changes immediately when game is active
  if (!gameOver) {
    // Move left
